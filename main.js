...
async function main() {
  const obj = await loadOBJ("/model/model.obj", "/model/model.mtl");
  ...
  function render() {
    gl.clearColor(0.2, 0.2, 0.2, 1.0); // Latar belakang abu-abu
    ...
    for (const [materialName, group] of Object.entries(obj.materialGroups)) {
        const material = obj.materials[materialName] || {
            ambient: [0.4, 0.4, 0.4],  // Ambient abu-abu lembut
            diffuse: [0.8, 0.8, 0.8],  // Putih terang
            specular: [1.0, 1.0, 1.0], // Highlight specular putih bersinar
            shininess: 32,             // Fokus specular sedang
            opacity: 1.0               // Tidak transparan 
        };
        ...
        // Tambahkan rim light untuk highlight di tepi objek
        gl.uniform1f(uniforms.uRimLightIntensity, 0.5); // Intensitas rim light
        gl.uniform3fv(uniforms.uRimLightColor, [1.0, 1.0, 1.0]); // Putih
